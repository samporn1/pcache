...
export class DefineRange extends Component {
  ...
  render () {
    ...
    const cross = matches.reduce((prev, m) => {
      ...
      const arr = [...new Array(len)].map((_, i) => ({[m]: first + i}))
    ...
    cross.forEach(c => {
      c[$key] = Object.entries(c).map(([k, v]) => `${k}:${v}`).join('-')
    })

    console.log('cross:', cross)

    const input = (name, display) => <div>
      <label>{display} <input value={state[name]||''} onChange={e => setExternalState({[name]: e.target.value})}/></label>
    </div>
    ...
    return <main className="define-range-main">
      <textarea placeholder="template" value={template} onChange={e => setExternalState({template: e.target.value})} />

      <div className="matches">
        {matches.map(m =><div key={m}>
          <h2>{m}</h2>
          {input(m+'$first', 'First')}
          {input(m+'$last', 'Last')}
          {input(m+'$pad', 'Pad')}
      ...
      <input placeholder="set" value={set} onChange={e => setExternalState({set: e.target.value})} />
      <button onClick={() => this.addToDownloads(urls)}>Add to downloads</button>
      <ul>
